...
function getGitVersion() {
  if (process.env.SUPERRAG_RUNTIME === "docker") return "--";
  try {
 ...
 * as well as build features in SuperRAG directly for a specific model or capabilities.
 *
 * Disable with  {@link https://github.com/NTT-DATA/superrag?tab=readme-ov-file#telemetry--privacy|Disable Telemetry}
...
/**
 * Returns the user agent for the SuperRAG deployment.
 * @returns {string} The user agent.
 */
function getSuperRAGUserAgent() {
  const version = getDeploymentVersion() || "unknown";
  return `SuperRAG/${version}`;
}
