...
const PGVector = {
   ...
   * - Defaults to "superrag_vectors" if no table name is provided.
   * @returns {string}
   */
  tableName: () => process.env.PGVECTOR_TABLE_NAME || "superrag_vectors",
  ...
  validateExistingEmbeddingTableSchema: async function (pgClient, tableName) {
    ...
    if (result.rows.length === 0)
      throw new Error(
        `The table '${tableName}' was found but does not contain any columns or cannot be accessed by role. It cannot be used as an embedding table in SuperRAG.`
      );
   ...
   * so that superrag can manage the table directly.
   *
  ...
  /**
   * Check if the superrag embedding table exists in the database
   * @returns {Promise<boolean>}
   */
  ...
  /**
   * Reset the entire vector database table associated with superrag
