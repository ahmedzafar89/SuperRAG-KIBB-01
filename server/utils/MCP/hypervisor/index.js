...
/**
 * @class MCPHypervisor
 * @description A class that manages MCP servers found in the storage/plugins/superrag_mcp_servers.json file.
 ...
 * For example, if the tools requires `npx` then the context in which SuperRAG mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * SuperRAG will take care of porting MCP servers to agent-callable functions via @agent directive.
...
class MCPHypervisor {
  ...
  #setupConfigFile() {
    this.mcpServerJSONPath =
      process.env.NODE_ENV === "development"
        ? path.resolve(
            __dirname,
            `../../../storage/plugins/superrag_mcp_servers.json`
          )
        : path.resolve(
            process.env.STORAGE_DIR ??
              path.resolve(__dirname, `../../../storage`),
            `plugins/superrag_mcp_servers.json`
          );
   ...
   * fixes the PATH and accessible commands when running SuperRAG outside of Docker during development on Mac/Linux and in-container (Linux).
  ...
  async #buildMCPServerENV(server) {
    ...
    if (process.env.SUPERRAG_RUNTIME === "docker") {
      baseEnv = {
  ...
  async bootMCPServers() {
    ...
    for (const { name, server } of serverDefinitions) {
      if (
        server.superrag?.hasOwnProperty("autoStart") &&
        server.superrag.autoStart === false
      ) {
        this.log(
          `MCP server ${name} has superrag.autoStart property set to false, skipping boot!`
        );
        this.mcpLoadingResults[name] = {
          status: "failed",
          message: `MCP server ${name} has superrag.autoStart property set to false, boot skipped!`,
        };
        continue;
      }
