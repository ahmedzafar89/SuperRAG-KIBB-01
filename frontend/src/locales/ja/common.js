...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      title: "LLMの設定",
      description:
        "SuperRAGは多くのLLMプロバイダーと連携できます。これがチャットを処理するサービスになります。",
    },
    userSetup: {
      title: "ユーザー設定",
    ...
    survey: {
      title: "SuperRAGへようこそ",
      description:
        "SuperRAGをあなたのニーズに合わせて構築するためにご協力ください。任意です。",
      email: "メールアドレスは何ですか？",
      useCase: "SuperRAGを何に使用しますか？",
      useCaseWork: "仕事用",
      useCasePersonal: "個人用",
      useCaseOther: "その他",
      comment: "SuperRAGをどのように知りましたか？",
      commentPlaceholder:
    ...
    workspace: {
      ...
      description:
        "最初のワークスペースを作成して、SuperRAGを始めましょう。",
    },
  },
  common: {
  ...
  api: {
    title: "APIキー",
    description:
      "APIキーにより、プログラム経由でこのSuperRAGインスタンスにアクセスおよび管理できます。",
    link: "APIドキュメントを読む",
  ...
  llm: {
    title: "LLMの設定",
    description:
      "これは、お好みのLLMチャットおよび埋め込みプロバイダー用の認証情報と設定です。これらのキーが最新かつ正確でない場合、SuperRAGは正しく動作しません。",
    provider: "LLMプロバイダー",
    providers: {
      azure_openai: {
  ...
  transcription: {
    ...
    "warn-start":
      "RAMやCPUが限られたマシンでローカルのWhisperモデルを使用すると、メディアファイルの処理中にSuperRAGが停止する可能性があります。",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "埋め込みとは、テキストをベクトルに変換するプロセスです。これらの認証情報は、ファイルやプロンプトをSuperRAGが処理できるフォーマットに変換するために必要です。",
    provider: {
      title: "埋め込みプロバイダー",
    },
  },
  ...
  vector: {
    title: "ベクターデータベース設定",
    description:
      "これは、SuperRAGインスタンスの動作方法用の認証情報と設定です。これらのキーが最新で正確であることが重要です。",
    provider: {
  ...
  privacy: {
    title: "プライバシーとデータ処理",
    description:
      "これは、接続されているサードパーティプロバイダーとSuperRAGがデータをどのように処理するかの設定です。",
    llm: "LLM選択",
    embedding: "埋め込み設定",
    vector: "ベクターデータベース",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "これらのファイルは、このSuperRAGインスタンス上のドキュメント処理機能にアップロードされます。第三者に送信・共有されることはありません。",
    },
    pinning: {
      what_pinning: "ドキュメントのピン留めとは？",
      pin_explained_block1:
        "SuperRAGでドキュメントを<b>ピン留め</b>すると、その内容全体がプロンプトウィンドウに挿入され、LLMがしっかり理解できるようになります。",
    ...
    watching: {
      ...
      watch_explained_block1:
        "SuperRAGでドキュメントを<b>ウォッチ</b>すると、元のソースから定期的に内容が<i>自動的に</i>同期されます。管理しているすべてのワークスペースで内容が自動更新されます。",
  ...
  "main-page": {
    ...
    checklist: {
      title: "はじめに",
      tasksLeft: "残りのタスク",
      completed: "SuperRAGの達人への道を進んでいます！",
      dismiss: "閉じる",
      tasks: {
        create_workspace: {
          title: "ワークスペースを作成する",
  ...
  security: {
    ...
    password: {
      title: "パスワード保護",
      description:
        "SuperRAGインスタンスをパスワードで保護します。これを忘れた場合、回復方法はないため、このパスワードを必ず保存してください。",
