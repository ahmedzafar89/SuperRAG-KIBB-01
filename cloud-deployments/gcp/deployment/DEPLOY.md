...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create superrag-deployment --config gcp/deployment/gcp_deploy_superrag.yaml
...
```
gcloud compute instances get-serial-port-output superrag-instance 
```

ssh into the instance

```
gcloud compute ssh superrag-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete superrag-deployment 
```
