# Run SuperRAG in production without Docker
...
Here you can find the scripts and known working process to run SuperRAG outside of a Docker container.
...
`git clone git@github.com:NTT-DATA/superrag.git`

2. `cd superrag` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
SuperRAG is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
SuperRAG should now be running on `http://localhost:3001`!

## Updating SuperRAG

To update SuperRAG with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple SuperRAG processes on the same instance as this can cause conflicts.
...
cd $HOME/superrag/frontend && yarn && yarn build && cd $HOME/superrag
...
cd $HOME/superrag/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/superrag/server && yarn
cd $HOME/superrag/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/superrag/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/superrag/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/superrag/collector
