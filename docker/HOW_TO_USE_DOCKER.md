# How to use Dockerized SuperRAG

Use the Dockerized version of SuperRAG for a much faster and complete startup of SuperRAG.

### Minimum Requirements

> [!TIP]
> Running SuperRAG on AWS/GCP/Azure?
...
\*SuperRAG by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*SuperRAG by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized SuperRAG!
...
> the docker container using SuperRAG as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/superrag
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run SuperRAG in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/superrag && \
...
$env:STORAGE_LOCATION="$HOME\Documents\superrag"; `
...
services:
  superrag:
    ...
    volumes:
      - superrag_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using SuperRAG! All your data and progress will persist between
...
- `git clone` this repo and `cd superrag` to get to the root directory.
- `touch server/storage/superrag.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of SuperRAG easier.

### Use the Midori AI Subsystem to Manage SuperRAG

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the SuperRAG docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in SuperRAG.
